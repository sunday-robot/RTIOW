...
inline bool box_compare(const shared_ptr<hittable> a, const shared_ptr<hittable> b, int axis) {
    ...
    if (!a->bounding_box(0, box_a) || !b->bounding_box(0, box_b))
...
bvh_node::bvh_node(
    ...
    if (object_span == 1) {
        ...
        left = make_shared<bvh_node>(objects, start, mid, exposureTime);
        right = make_shared<bvh_node>(objects, mid, end, exposureTime);
    }

    aabb box_left, box_right;

    if (  !left->bounding_box (exposureTime, box_left)
       || !right->bounding_box(exposureTime, box_right)
    )
        std::cerr << "No bounding box in bvh_node constructor.\n";
